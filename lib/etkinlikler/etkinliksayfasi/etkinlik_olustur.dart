                            ...
                            onPressed: () {
                              if(_userModel.user.uyelikTipi=="standart"){
                                var dialogBilgi = AlertBilgilendirme(
                                  message: "Standart Paketde Turnuva Oluşturamazsınız.",
                                  onPostivePressed: (){
...
//                        onPressed: () {
//                          print("Google api: ${getGoogleApiKey()}");
...
//                                onMapCreated: (value){
//                                  print("onMapCreated Map ID: ${value.mapId}");
//                                },
//                                onGeocodingSearchFailed: (value){
//                                  print("onGeocodingSearchFailed: $value");
//                                },
//                                region: "tr",
...
//                        },
                  ...
                  Container(
                    ...
                    child: RaisedButton(
                      ...
                      onPressed: () async {
                       if(_userModel.user.uyelikTipi=="standart"){
                         if(_userModel.user.acilanetkinliksayim>2){
                           var dialogBilgi = AlertBilgilendirme(
                             message: "Standart Paketde haftalık en fazla 2 etkinlik açabilirsiniz.",
                             onPostivePressed: (){
                               Navigator.pop(context);
                             },
                           );
                           ...
                           etkinlikOlustur();
                           FirebaseFirestore.instance.collection("users").doc(_userModel.user.userId).update({'acilanetkinliksayisi':FieldValue.increment(1)});
                         }
                       }else if(_userModel.user.uyelikTipi=="plus"){
                         if(_userModel.user.acilanetkinliksayim>5){
                           var dialogBilgi = AlertBilgilendirme(
                             message: "Plus Paketde haftalık en fazla 5 etkinlik açabilirsiniz.",
                             onPostivePressed: (){
                               Navigator.pop(context);
                             },
                           );
  ...
  Future<void> etkinlikOlustur() async {
    ...
    userCollection.doc(_userModel.user.userId).collection("etkinliklerim").doc(etkinlikCollection.id).set(etkinliklerim);
  ...
  bool validateForm() {
    ...
    var selectedDateAndTime = DateTime(selectedDate.year, selectedDate.month, selectedDate.day, selectedTime.hour, selectedTime.minute);
    ...
    print("Seçilen tarih: "+selectedDateAndTime.difference(DateTime.now()).isNegative.toString());
    ...
    if (kacKisiController.text.isEmpty) {
      Fluttertoast.showToast(
          msg: "Lütfen Kaç Kişi Katılması Gerektiğini Giriniz.",
          toastLength: Toast.LENGTH_LONG,
          gravity: ToastGravity.CENTER,
          textColor: Colors.white,
          backgroundColor: Theme.of(context).primaryColor,
